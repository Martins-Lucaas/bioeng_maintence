...
class _RessonanciaMagneticaPageState extends State<RessonanciaMagneticaPage> {
  ...
  void _saveCompletionStatus() async {
    if (_signatureController.isNotEmpty) {
      ...
      if (signatureData != null) {
        _databaseReference.child('setores/ressonancia_magnetica').set({
          'finalizado': true,
  ...
  void _loadCompletionStatus() {
    _databaseReference.child('setores/ressonancia_magnetica').once().then((DatabaseEvent event) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Ressonância Magnética'),
      ),
      body: Padding(

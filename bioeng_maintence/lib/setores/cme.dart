...
class _CMEPageState extends State<CMEPage> {
  ...
  final String _qrData = 'app://setor/cme'; // QR code fixo para deep link CME
  ...
  void _loadCompletionStatus() {
    _databaseReference.child('setores/cme/finalizado').once().then((DatabaseEvent event) {
      if (event.snapshot.exists) {
        setState(() {
  ...
  void _saveCompletionStatus() async {
    if (_signatureController.isNotEmpty) {
      ...
      if (signatureData != null) {
        ...
        _databaseReference.child('setores/cme').set({
          'finalizado': true,
          'assinatura': {

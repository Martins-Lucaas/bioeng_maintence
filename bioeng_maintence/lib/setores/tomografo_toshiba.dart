...
class TomografoToshibaPage extends StatefulWidget {
  const TomografoToshibaPage({super.key});

  @override
  State<TomografoToshibaPage> createState() => _TomografoToshibaPageState();
}

class _TomografoToshibaPageState extends State<TomografoToshibaPage> {
  ...
  final String _qrData = 'app://setor/tomografo_toshiba';  // QR code fixo para deep link
  ...
  void _loadCompletionStatus() {
    _databaseReference.child('setores/tomografo_toshiba/finalizado').once().then((DatabaseEvent event) {
      if (event.snapshot.exists) {
        setState(() {
  ...
  void _saveCompletionStatus() async {
    if (_signatureController.isNotEmpty) {
      ...
      if (signatureData != null) {
        ...
        _databaseReference.child('setores/tomografo_toshiba').set({
          'finalizado': true,
          'assinatura': {
  ...
  Future<void> _saveQRCodeAsPDF() async {
    ...
    await Printing.sharePdf(
      ...
      filename: 'qr_code_tomografo_toshiba.pdf',
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Tomógrafo Toshiba'),
        leading: IconButton(

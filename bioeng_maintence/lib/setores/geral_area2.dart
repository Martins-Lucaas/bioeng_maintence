...
class GeralArea2Page extends StatefulWidget {
  const GeralArea2Page({super.key});

  @override
  State<GeralArea2Page> createState() => _GeralArea2PageState();
}

class _GeralArea2PageState extends State<GeralArea2Page> {
  ...
  final String _qrData = 'app://setor/geral_area2'; // QR code fixo para deep link Geral Área 2
  ...
  void _loadCompletionStatus() {
    _databaseReference.child('setores/geral_area2/finalizado').once().then((DatabaseEvent event) {
      if (event.snapshot.exists) {
        setState(() {
  ...
  void _saveCompletionStatus() async {
    if (_signatureController.isNotEmpty) {
      ...
      if (signatureData != null) {
        ...
        _databaseReference.child('setores/geral_area2').set({
          'finalizado': true,
          'assinatura': {
  ...
  Future<void> _saveQRCodeAsPDF() async {
    ...
    await Printing.sharePdf(
      ...
      filename: 'qr_code_geral_area2.pdf',
    );
  }

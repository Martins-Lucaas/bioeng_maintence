...
class ProntoSocorroPage extends StatefulWidget {
  const ProntoSocorroPage({super.key});

  @override
  State<ProntoSocorroPage> createState() => _ProntoSocorroPageState();
}

class _ProntoSocorroPageState extends State<ProntoSocorroPage> {
  ...
  final String _qrData = 'app://setor/pronto_socorro';  // QR code fixo para deep link do Pronto Socorro
  ...
  void _loadCompletionStatus() {
    _databaseReference.child('setores/pronto_socorro/finalizado').once().then((DatabaseEvent event) {
      if (event.snapshot.exists) {
        setState(() {
  ...
  void _saveCompletionStatus() async {
    if (_signatureController.isNotEmpty) {
      ...
      if (signatureData != null) {
        ...
        _databaseReference.child('setores/pronto_socorro').set({
          'finalizado': true,
          'assinatura': {
  ...
  Future<void> _saveQRCodeAsPDF() async {
    ...
    await Printing.sharePdf(
      ...
      filename: 'qr_code_pronto_socorro.pdf',
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Pronto Socorro'),
        leading: IconButton(

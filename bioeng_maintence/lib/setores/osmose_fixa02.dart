...
class OsmoseFixa02Page extends StatefulWidget {
  const OsmoseFixa02Page({super.key});

  @override
  State<OsmoseFixa02Page> createState() => _OsmoseFixa02PageState();
}

class _OsmoseFixa02PageState extends State<OsmoseFixa02Page> {
  ...
  final String _qrData = 'app://setor/osmose_fixa02';  // QR code fixo para deep link Osmose Fixa 02
  ...
  void _loadCompletionStatus() {
    _databaseReference.child('setores/osmose_fixa02/finalizado').once().then((DatabaseEvent event) {
      if (event.snapshot.exists) {
        setState(() {
  ...
  void _saveCompletionStatus() async {
    if (_signatureController.isNotEmpty) {
      ...
      if (signatureData != null) {
        ...
        _databaseReference.child('setores/osmose_fixa02').set({
          'finalizado': true,
          'assinatura': {
  ...
  Future<void> _saveQRCodeAsPDF() async {
    ...
    await Printing.sharePdf(
      ...
      filename: 'qr_code_osmose_fixa02.pdf',
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Osmose Fixa 02'),
        leading: IconButton(

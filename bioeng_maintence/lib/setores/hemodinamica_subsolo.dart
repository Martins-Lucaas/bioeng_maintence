...
class HemodinamicaSubsoloPage extends StatefulWidget {
  const HemodinamicaSubsoloPage({super.key});

  @override
  State<HemodinamicaSubsoloPage> createState() => _HemodinamicaSubsoloPageState();
}

class _HemodinamicaSubsoloPageState extends State<HemodinamicaSubsoloPage> {
  ...
  final String _qrData = 'app://setor/hemodinamica_subsolo'; // QR code fixo para deep link Hemodinâmica Subsolo
  ...
  void _loadCompletionStatus() {
    _databaseReference.child('setores/hemodinamica_subsolo/finalizado').once().then((DatabaseEvent event) {
      if (event.snapshot.exists) {
        setState(() {
  ...
  void _saveCompletionStatus() async {
    if (_signatureController.isNotEmpty) {
      ...
      if (signatureData != null) {
        ...
        _databaseReference.child('setores/hemodinamica_subsolo').set({
          'finalizado': true,
          'assinatura': {
  ...
  Future<void> _saveQRCodeAsPDF() async {
    ...
    await Printing.sharePdf(
      ...
      filename: 'qr_code_hemodinamica_subsolo.pdf',
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Hemodinâmica Subsolo'),
        leading: IconButton(

...
class HemodinamicaCCPage extends StatefulWidget {
  const HemodinamicaCCPage({super.key});

  @override
  State<HemodinamicaCCPage> createState() => _HemodinamicaCCPageState();
}

class _HemodinamicaCCPageState extends State<HemodinamicaCCPage> {
  ...
  final String _qrData = 'app://setor/hemodinamica_cc'; // QR code fixo para deep link Hemodinâmica CC
  ...
  void _loadCompletionStatus() {
    _databaseReference.child('setores/hemodinamica_cc/finalizado').once().then((DatabaseEvent event) {
      if (event.snapshot.exists) {
        setState(() {
  ...
  void _saveCompletionStatus() async {
    if (_signatureController.isNotEmpty) {
      ...
      if (signatureData != null) {
        ...
        _databaseReference.child('setores/hemodinamica_cc').set({
          'finalizado': true,
          'assinatura': {
  ...
  Future<void> _saveQRCodeAsPDF() async {
    ...
    await Printing.sharePdf(
      ...
      filename: 'qr_code_hemodinamica_cc.pdf',
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Hemodinâmica CC'),
        leading: IconButton(

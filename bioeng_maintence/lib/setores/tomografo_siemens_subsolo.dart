...
class TomografoSiemensSubsoloPage extends StatefulWidget {
  const TomografoSiemensSubsoloPage({super.key});

  @override
  State<TomografoSiemensSubsoloPage> createState() => _TomografoSiemensSubsoloPageState();
}

class _TomografoSiemensSubsoloPageState extends State<TomografoSiemensSubsoloPage> {
  ...
  final String _qrData = 'app://setor/tomografo_siemens_subsolo';  // QR code fixo para deep link
  ...
  void _loadCompletionStatus() {
    _databaseReference.child('setores/tomografo_siemens_subsolo/finalizado').once().then((DatabaseEvent event) {
      if (event.snapshot.exists) {
        setState(() {
  ...
  void _saveCompletionStatus() async {
    if (_signatureController.isNotEmpty) {
      ...
      if (signatureData != null) {
        ...
        _databaseReference.child('setores/tomografo_siemens_subsolo').set({
          'finalizado': true,
          'assinatura': {
  ...
  Future<void> _saveQRCodeAsPDF() async {
    ...
    await Printing.sharePdf(
      ...
      filename: 'qr_code_tomografo_siemens_subsolo.pdf',
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Tomógrafo Siemens Subsolo'),
        leading: IconButton(

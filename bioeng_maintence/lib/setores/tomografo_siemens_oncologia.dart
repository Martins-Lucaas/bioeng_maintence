...
class _TomografoSiemensOncologiaPageState extends State<TomografoSiemensOncologiaPage> {
  ...
  final String _qrData = 'app://setor/tomografo_siemens_oncologia';  // QR code fixo para deep link
  ...
  void _loadCompletionStatus() {
    _databaseReference.child('setores/tomografo_siemens_oncologia/finalizado').once().then((DatabaseEvent event) {
      if (event.snapshot.exists) {
        setState(() {
  ...
  Future<void> _saveQRCodeAsPDF() async {
    ...
    await Printing.sharePdf(
      ...
      filename: 'qr_code_tomografo_siemens_oncologia.pdf',
    );
  }
